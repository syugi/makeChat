  ...
  prjList.forEach((data)=>{
    ...
    list += `<div class="select-none cursor-pointer bg-gray-200 rounded-md flex flex-1 items-center p-4  transition duration-500 ease-in-out transform hover:-translate-y-1 hover:shadow-lg">
                ...
                <div class="flex-1 pl-1 mr-16">
                  <div class="font-medium" id="prjNm">${data.PRJ_NM}</div>
                  <div class="text-gray-600 text-sm">${data.PRJ_DESC}</div>
                </div>
                <div class="text-gray-600 text-xs btn_modify_prj" id=${data.PRJ_ID}>수정</div>
                <div class="text-gray-600 text-xs btn_delete_prj" id=${data.PRJ_ID}>삭제</div>
    ...
    <div id="projectModal" style=" background-color: rgba(0, 0, 0, 0.8)" class="fixed z-40 top-0 right-0 left-0 bottom-0 h-full w-full hidden">
          ...
          <form method="POST" onsubmit="return validate();" action="/save">
              <div class="mb-4">
                ...
                <input class="mb-1 bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" id="inline-full-name" type="text" id="prjNm" name="prjNm" value="NEW PROJECT">
              </div>

              <div class="mb-4">
                <label class="text-gray-800 block mb-1 font-bold text-sm uppercase tracking-wide">설명</label>
                <input class="mb-1 bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" id="inline-full-name" type="text" id="prjDesc" name="prjDesc">
              </div>
