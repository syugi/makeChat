...
function main(prjData,profData,chatData){
 ...
    list_prof += `      <img class="profImg shadow-md rounded-full w-full h-full object-cover ${active}" src="../uploads/${data.FILE_PATH}" alt="" onclick="selectProf( this,'${data.PROF_ID}')" ondblclick="openModalProf('${data.PROF_ID}')">`;
		list_prof += `			</div>`;
...
<input name="img_file" id="fileInputProf" accept="image/*" class="hidden" type="file" onchange="handleProfFile(event);">
              ...
              <div class="mb-4">
								...
								<div class="mb-4">
											...
											<label class="inline-flex items-center">
												<input type="radio" class="form-radio" id="mopdalPosition" name="position" value="left">
												<span class="ml-2">왼쪽</span>
											</label>
											<label class="inline-flex items-center ml-6">
												<input type="radio" class="form-radio" id="mopdalPosition" name="position" value="right" checked>
